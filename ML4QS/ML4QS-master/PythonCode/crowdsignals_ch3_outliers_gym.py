...
try:
    dataset = pd.read_csv(dataset_path + 'chapter2_result_gym.csv', index_col=0)
except IOError as e:
...
# plt.hist(dataset['acc_phone_x'], edgecolor='black',linewidth=1)
# plt.show()
# exit()
# Step 1: Let us see whether we have some outliers we would prefer to remove.

# Determine the columns we want to experiment on.
outlier_columns = ['gyr_phone_y']#'''acc_phone_x', 'mag_phone_x']
...
#     try:
#         dataset = OutlierDist.simple_distance_based(dataset, [col], 'euclidean', 0.20, 0.99)
...
# We take Chauvent's criterion and apply it to all but the label data... Nope, SDBA!
# exit()
...
dataset.to_csv(dataset_path + 'chapter3_result_outliers_gym.csv')
