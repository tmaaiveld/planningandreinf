...
dataset = pd.read_csv(dataset_path + 'chapter3_result_outliers_gym.csv', index_col=0)
...
#
# original_dataset = pd.read_csv(dataset_path + 'chapter2_result_gym.csv', index_col=0)
...
# DataViz.plot_imputed_values(kalman_dataset, ['original', 'kalman'], 'gyr_phone_y', kalman_dataset['acc_phone_x_kalman'])
# exit()
...
# # Let us study acc_phone_x:  # no, lets study filter effect on all variables
# variables = ['acc_phone_x', 'acc_phone_y', 'acc_phone_z',  'gyr_phone_x', 'gyr_phone_y',
#                          'gyr_phone_z',   'mag_phone_x', 'mag_phone_y', 'mag_phone_z']
# for variable in variables:
#     new_dataset = LowPass.low_pass_filter(copy.deepcopy(dataset), variable, fs, cutoff, order=10)
#     DataViz.plot_dataset(new_dataset.ix[int(0.4*len(new_dataset.index)):int(0.43*len(new_dataset.index)), :], [variable,  variable + '_lowpass'], ['exact','exact'], ['line', 'line'])
# exit()
...
# We select 7 as the best number of PC's as this explains most of the variance   # for us, 4
...
DataViz.plot_dataset(dataset, ['acc_', 'gyr_',  'mag_', 'pca_', 'label'], ['like', 'like', 'like', 'like','like', 'like'], ['line', 'line', 'line', 'line', 'points', 'points'])

# Store the outcome.

dataset.to_csv(dataset_path + 'chapter3_result_final_gym.csv')