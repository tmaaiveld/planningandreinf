...
try:
    dataset = pd.read_csv(dataset_path + 'chapter3_result_final_gym.csv', index_col=0)
except IOError as e:
...
ws  = int(float(0.5*60000)/milliseconds_per_instance)  #  Let's do ws =  100 instances  (=100*0.3 seconds = 30 seconds) (the middle, 2nd element in the window_sizes list above)
...
dataset = CatAbs.abstract_categorical(dataset, ['label'], ['like'], 0.03, int(float(120000)/milliseconds_per_instance), 2) # Let's do ws = 400 instances (=2 minutes)
...
data_table = FreqAbs.abstract_frequency(copy.deepcopy(dataset), ['acc_phone_x'], int(float(10000)/milliseconds_per_instance), fs) # windowsize   = 20 instances  = 6 seconds
...
exit()
dataset = FreqAbs.abstract_frequency(dataset, periodic_predictor_cols, int(float(6000)/milliseconds_per_instance), fs)
...
dataset.to_csv(dataset_path + 'chapter4_result_gym.csv')

DataViz.plot_dataset(dataset, ['acc_phone_x', 'gyr_phone_x',  'mag_phone_x', 'pca_1', 'label'], ['like', 'like', 'like', 'like', 'like','like'], ['line', 'line', 'line', 'line', 'line', 'points'])
