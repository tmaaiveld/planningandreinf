...
try:
    dataset = pd.read_csv(dataset_path + 'chapter4_result_gym.csv', index_col=0)
except IOError as e:
...
feats = [['acc_phone_x', 'acc_phone_y', 'acc_phone_z'],
               ['gyr_phone_x', 'gyr_phone_y', 'gyr_phone_z'],
               ['mag_phone_x','mag_phone_y','mag_phone_z']]

dataset_knn = copy.deepcopy(dataset)
names = ['acc_cluster', 'gyr_cluster', 'mag_cluster']
i=0

for feat in feats:
    print 'running for ' + feat[0]
    for k in k_values:
        print 'k = ', k
        dataset_cluster = clusteringNH.k_means_over_instances(names[i], copy.deepcopy(dataset),  feat, k, 'default', 20, 10)
    ...
    dataset_knn = clusteringNH.k_means_over_instances(names[i], copy.deepcopy(dataset_knn), feat, k, 'default', 50, 50)
    # DataViz.plot_clusters_3d(dataset_knn, feat, 'cluster', ['label'])
    # DataViz.plot_silhouette(dataset_knn, 'cluster', 'silhouette')
    # util.print_latex_statistics_clusters(dataset_knn, 'cluster', feat, 'label')
...
dataset_knn.to_csv(dataset_path + 'chapter5_result_gym.csv')
