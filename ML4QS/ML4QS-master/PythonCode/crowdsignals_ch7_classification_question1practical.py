...
# time_features = [name for name in dataset.columns if (('_temp_' in name) and ('label' not in name))]
...
# features, ordered_features, ordered_scores = fs.forward_selection(15,
#                                                                   train_X[features_after_chapter_5].fillna(method='ffill').fillna(method='bfill'),
#                                                                   train_y)   # Missing values had to be filled in elsewhere too! I have extracted important features from this function and now the  split_single_dataset_classification() handles filling in missing values.
...
# plot.plot(range(1, 16), ordered_scores)
...
###  Copy paste the selected_features and define them: ###
selected_features = ['acc_phone_y_freq_0.0_Hz_ws_40', 'press_phone_pressure_temp_mean_ws_120', 'mag_phone_y_temp_mean_ws_120',
                     'acc_watch_y_temp_mean_ws_120', 'gyr_watch_x_temp_std_ws_120', 'mag_phone_z_temp_mean_ws_120', 'acc_watch_y_pse',
                     'acc_watch_x_freq_1.2_Hz_ws_40', 'gyr_watch_x_freq_1.9_Hz_ws_40', 'acc_watch_y_freq_0.5_Hz_ws_40']
...
repeats = 5
# print train_X[features_after_chapter_4 ].columns.values
scores_over_all_algs = []

for i in range(0, len(possible_feature_sets)):
    selected_train_X = train_X[possible_feature_sets[i]]
    selected_test_X = test_X[possible_feature_sets[i]]
    # print train_X[possible_feature_sets[-1]].colums.values
    ...
    for repeat in range(0, repeats):
        print 'Running for non deterministic classifiers, this is iteration: '
        print str(repeat)
        print '\n'
